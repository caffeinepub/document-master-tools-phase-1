...
export default function GPACalculator() {
  ...
  const getTotalCredits = () => {
    return courses
      .reduce((sum, c) => sum + (Number.parseFloat(c.credits) || 0), 0)
      .toFixed(1);
  };
      ...
      <Card className="bg-card border-border">
        <CardContent className="pt-6 space-y-6">
          <div>
            ...
            <RadioGroup
              value={scale}
              onValueChange={(value) => setScale(value as "4.0" | "10.0")}
            >
              <div className="flex gap-4">
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="4.0" id="scale-4" />
                  <Label htmlFor="scale-4" className="cursor-pointer">
                    4.0 Scale
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="10.0" id="scale-10" />
                  <Label htmlFor="scale-10" className="cursor-pointer">
                    10.0 Scale
                  </Label>
                </div>
              </div>
