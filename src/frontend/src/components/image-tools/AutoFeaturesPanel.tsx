...
export default function AutoFeaturesPanel({
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      // 2. Compress
      if (compressEnabled) {
        canvas = await compressToKB(canvas, Number.parseInt(compressKB));
      }
  ...
  const anyEnabled =
    whiteBackground || compressEnabled || (multiCopy && isProUser);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <label className="flex items-start gap-3 p-3 rounded-lg border border-border bg-muted/20 cursor-pointer hover:bg-muted/40 transition-colors">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Replace transparent or near-white borders with solid white
            </p>
          </div>
        </label>
        ...
        <div className="flex items-start gap-3 p-3 rounded-lg border border-border bg-muted/20">
          ...
          <div>
            <p className="text-sm font-medium text-foreground">
              Auto Center Crop Guide
            </p>
            <p className="text-xs text-muted-foreground">
              Toggle the crop guide overlay in the Preview panel using the{" "}
              <span className="font-medium text-primary">Crop</span> button
              above
            </p>
          </div>
        </div>
        ...
        {isProUser ? (
          <label className="flex items-start gap-3 p-3 rounded-lg border border-border bg-muted/20 cursor-pointer hover:bg-muted/40 transition-colors">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Arrange 4 copies on A4 300 DPI canvas (2480×3508 px)
              </p>
            </div>
          </label>
        ) : (
