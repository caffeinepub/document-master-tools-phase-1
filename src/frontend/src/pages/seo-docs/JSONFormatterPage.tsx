...
type Status = { type: "valid" | "invalid" | "idle"; message: string };

function formatJSON(input: string): { output: string; status: Status } {
  if (!input.trim())
...
function minifyJSON(input: string): { output: string; status: Status } {
  if (!input.trim())
    return { output: "", status: { type: "idle", message: "" } };
  try {
    const parsed = JSON.parse(input);
...
function validateJSON(input: string): Status {
...
export default function JSONFormatterPage({ onNavigate, onBack }: Props) {
    ...
    >
      ...
      <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
        ...
        >
          ...
          <span style={{ margin: "0 0.4rem" }}>›</span>
          <span style={{ color: ACCENT_LIGHT }}>JSON Formatter Online</span>
        </nav>
        ...
        >
          Paste your JSON to instantly format it into a readable structure,
          minify it for production, or validate its syntax. All processing is
          done in your browser — your data never leaves your device.
        </p>
        ...
        <div style={cardStyle}>
          ...
          {status.message && (
            <div
              data-ocid={
                status.type === "valid"
                  ? "jsonformat.success_state"
                  : "jsonformat.error_state"
              }
              style={{
        ...
        {/* ── SEO CONTENT ── */}
        <div style={cardStyle}>
          <h2 style={h2Style}>What Is JSON and Why Format It?</h2>
          <p style={bodyText}>
            JSON (JavaScript Object Notation) is the universal language of web
            APIs. When an application fetches data from a REST API, the response
            almost always arrives as JSON. However, the JSON returned by most
            APIs is minified — all the whitespace has been stripped out to
            reduce file size. Minified JSON looks like this:{" "}
            <code
              style={{
                background: "#0f172a",
          ...
          <p style={{ ...bodyText, marginTop: "0.75rem" }}>
            Formatting (or prettifying) JSON adds proper indentation and line
            breaks to reveal the data structure. A formatted version of the same
            data is immediately scannable: you can see object keys, nested
            arrays, value types, and data hierarchy at a glance. This is
            essential when debugging API responses, reviewing configuration
            files, analyzing webhook payloads, or working with any data stored
            in JSON format.
          </p>
          <p style={{ ...bodyText, marginTop: "0.75rem" }}>
            JSON validation is equally important. A single missing comma, an
            unmatched bracket, or an extra trailing comma will make the entire
            JSON document invalid. Our validator parses the JSON using the
            browser's native engine and returns the exact error description,
            helping you fix syntax issues in seconds rather than hunting through
            thousands of characters manually.
          </p>
          <p style={{ ...bodyText, marginTop: "0.75rem" }}>
            Common JSON use cases include: REST API response analysis,
            configuration files (package.json, .eslintrc.json), data exports
            from databases, webhook payload debugging, Postman/Insomnia response
            inspection, and migration data review. Our tool handles all of these
            use cases with zero setup.
          </p>
        </div>

        <div style={cardStyle}>
          <h2 style={h2Style}>How to Format JSON Online</h2>
          <div
        ...
        <div style={cardStyle}>
          <h2 style={h2Style}>Tips for Working with JSON</h2>
          <ul
            style={{
              listStyle: "none",
              padding: 0,
